...
if [ -z "${WORDPRESS_AUTHOR_PASSWORD:-}" ] && [ -f /run/secrets/wp_author_password ]; then
  ...
  echo "WORDPRESS_AUTHOR_PASSWORD: $WORDPRESS_AUTHOR_PASSWORD"
fi
...
# 後で消す
echo "mysqladmin のコマンドで exit code が正しく拾えてるかを一度試してみる"
mysqladmin ping -h"$WORDPRESS_DB_HOST" -u"$WORDPRESS_DB_USER" -p"$WORDPRESS_DB_PASSWORD" --silent && echo "ok" || echo "ng"
